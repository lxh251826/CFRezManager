...
LTC 文件支持缩略图和双击查看。程序会优先使用内置解码器处理普通 LTC、LZMA 压缩 LTC，以及 CrossFire 常见的带 `54 83 B2 E1` 头和外层 XOR 的 LTC；解码后的 LTA 文本可以直接在文本预览窗口查看。如果内容是 LithTech 模型，会尝试渲染模型缩略图，并可打开独立模型预览窗口。仍无法识别的 LTC 可以通过环境变量 `CFREZ_LTC_TO_LTA` 或 `tools` 目录下的外部转换器作为兜底。

DAT 文件支持 CrossFire 常见地图和对象预览。LithTech world DAT v85 可以渲染为地图模型缩略图，并可双击打开独立模型预览窗口；普通 DAT 和 LZMA 压缩 DAT 都会自动识别，LZMA 压缩资源会在缩略图右上角显示 `LZMA` 标记。CrossFire 对象 DAT 会解码为文本预览，当前支持区域 `Zoneman`、环境音效 `EnvSound`、移动路径 `MovePath` 以及过场相机 `CameraAnimation` 数据。
...
- `CrossFireLtcDecoder.cs` / `LithTechLtcNativeDecoder.cs`：LTC 文本和模型预览解码逻辑。
- `CrossFireDatDecoder.cs` / `LithTechWorldDatDecoder.cs`：DAT 对象文本和 LithTech world 地图预览解码逻辑。
